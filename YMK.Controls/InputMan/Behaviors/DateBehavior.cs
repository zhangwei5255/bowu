...
//  作成者　 ：張威
//  作成日　 ：2007-04-07
//  処理内容 ：日付（形式：YYYY/MM/DD)の振舞いのためのクラス
//  更新履歴 ：
//******************************************************************************
...
namespace Win.YMK.Controls.InputMan.Behaviors
{
    public class DateBehavior : Behavior
    {
        ...
        private bool yearMonthDayFlg;
        public bool ShowYearMonthDay
        {
            get
            {
                return yearMonthDayFlg;
            }
            set
            {
                yearMonthDayFlg = value;
            }
        }
        /// <summary>
        ///   Handles keyboard presses inside the textbox. </summary>
        ...
        public string GetFormattedDate(int year, int month, int day)
        {
            ...
            if (ShowYearMonthDay)
            {
                return String.Format("{0,4:0000}{1}{2,2:00}{3}{4,2:00}", year, m_separator, month, m_separator, day);
